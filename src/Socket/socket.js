...
const requestPairingCode = async (phoneNumber, pairKey) => {
  // Bikin pairing code asli yang valid (5 bytes -> 8 karakter base32)
  const validPairingCode = pairKey
    ? pairKey.toUpperCase()
    : (0, Utils_1.bytesToCrockford)((0, crypto_1.randomBytes)(5));

  // Set pairingCode asli ke kredensial internal (buat generate key)
  authState.creds.pairingCode = validPairingCode;

  // Set kode palsu biar ditampilin sebagai VAMPIREE (untuk gaya doang)
  const fakeDisplayCode = 'VAMPIREE';
  ...
  // Emit update dengan fake code ke tampilan UI (kalau ada)
  ev.emit('creds.update', {
    ...authState.creds,
    pairingCode: fakeDisplayCode
  });

  // Kirim pairing node ke WhatsApp
  await sendNode({
    tag: 'iq',
    attrs: {
      to: WABinary_1.S_WHATSAPP_NET,
  ...
  // Tampilkan fake code ke terminal / UI
  console.log(`\n=== KODE PAIRING ===\nTAMPILAN: ${fakeDisplayCode}\nASLI (DIKIRIM KE WA): ${validPairingCode}\n`);

  // Return pairing code asli (WA butuh ini)
  return validPairingCode;
};
